...
| 스프린트 | 주요 목표 | 핵심 결과물 | 예상 기간 |
| :--- | :--- | :--- | :--- |
| **Sprint 1** | **UI/UX 기반 및 비주얼 인터랙션 수립** | 배터리 슬라이더, 5단계 고양이 모션 매핑, 피로 원인 칩 UI, 800px 스케일 래퍼 | 1-2일 |
| **Sprint 2** | **LLM API 연동 및 데이터 파라인 구축** | Next.js `/api/prescribe` 라우트, GPT-4o-mini JSON 연동, 클라이언트 바인딩 | 1-2일 |
| **Sprint 3** | **7가지 예외 처리 및 Fallback 시스템 구현** | 태그 미선택/중복클릭/지연/에러/타임아웃/오프라인/극단수치 Fallback 정책 | 1-2일 |
| **Sprint 4** | **처방 카터 인터랙션, QA 및 DoD 검증** | 체크리스트, 피드백 토스트, 리셋 애니메이션, DoD 최종 검증 | 1일 |
...
#### 1.2 세부 작업 항목 (Tasks)
- [ ] **[Layout] 단일 화면 구조 정립 (`app/page.tsx`, `app/layout.tsx`)**
  - 데스크톱 고정 뷰 중심 (가로 최소 800px 유지)
  - 가로 800px 미만 축소 시 CSS `transform: scale()` 적용 비율 정렬
- [ ] **[Visual] 5단계 배터리 & 고양이 비주얼 매핑 (`components/battery-cat.tsx`, `components/battery-slider.tsx`)**
...
- [ ] **[Input] 피로 원인 선택 칩 UI (`components/tag-chips.tsx`)**
...
#### 2.2 세부 작업 항목 (Tasks)
- [ ] **[Backend] Next.js API 라우트 생성 (`app/api/prescribe/route.ts`)**
...
- [ ] **[Contract] JSON Response Schema 검증**
...
- [ ] **[Frontend] 클라이언트 API 호출 및 로딩 처리 (`app/page.tsx`)**
...
#### 4.2 세부 작업 항목 (Tasks)
- [ ] **[Interaction] 1분 힐링 루틴 체크박스 (`components/result-card.tsx`)**
  - 루틴 항목 클릭 시 체크 표시 및 취소선(line-through) 시각적 피드백
- [ ] **[Interaction] 피드백 버튼 & 토스트 UI**
  - `[👍 도움이 됐어요]` / `[👎 별로예요]` 단일 선택 활성화
  - *"피드백이 전달되었습니다!"* 토스트 메시지 노출 (2.2초 후 자동 소멸)
- [ ] **[Interaction] [🔄 다시하기] 초기화 및 스크롤**
  - 슬라이더 50% 리셋, 원인 칩 해제
  - 결과 카드가 접히며(Slide-up) 상단 초기 화면으로 스무스 스크롤 복귀
- [ ] **[Audit] DoD 최종 체크리스트 검증**
  - [ ] 슬라이더 반응성 (5구간 비주얼 프레임 드랍 여부)
  - [ ] 유효성 검사 (태그 미선택 시 Red 점멸 & 툴팁)
  - [ ] 중복 방지 (버튼 비활성화 및 로딩 전환)
  - [ ] LLM 파싱 & 렌더링 (3초 이내 카드시각화)
  - [ ] Fallback 검증 (네트워크 차단/타임아웃/에러 시 100% 정적 데이터 보장)
  - [ ] 상태 초기화 ([다시하기] 리셋 동작)
  - [ ] In-Scope / Out-of-Scope 제약사항 엄수 (로그인/DB/결제/공유 기능 배제 확인)
...
| :--- | :--- | :---: |
| **슬라이더 반응성** | 0~100% 슬라이더 조작 시 5개 구간 고양이 표정/모션 실시간 매핑 | ⬜ |
| **유효성 검사** | 태그 미선택 시 API 차단, Red 점멸 애니메이션 및 안내 툴팁 동작 | ⬜ |
| **중복 방지** | 처방받기 클릭 즉시 `disabled`, 로딩 모션 전환 및 중복 호출 차단 | ⬜ |
| **LLM 연동** | 정상 API 응답 JSON 파싱 후 P95 2.5초 이내 처방 카드로 시각화 | ⬜ |
| **Fallback 보장** | 네트워크 차단, 5초 타임아웃, 5xx 에러 시 에러 노출 없이 정적 처방 출력 | ⬜ |
| **상태 초기화** | [다시하기] 클릭 시 슬라이더(50%) 및 원인 칩 초기화, 상단 스크롤 복귀 | ⬜ |
| **범위 제약 엄수** | 로그인, 결제, DB, 파일 업로드, SNS 공유 없는 독립 SPA 동작 | ⬜ |
