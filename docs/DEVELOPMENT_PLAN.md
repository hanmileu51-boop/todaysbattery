...
| **유효성 검사** | 태그 미선택 시 API 차단, Red 점멸 애니메이션 및 안내 툴팁 동작 | ⬜ (Sprint 3) |
| **중복 방지** | 처방받기 클릭 즉시 `disabled`, 로딩 모션 전환 및 중복 호출 차단 | ⬜ (Sprint 3) |
| **LLM 연동** | 정상 API 응답 JSON 파싱 후 P95 2.5초 이내 처방 카드로 시각화 | ✅ PASS |
| **Fallback 보장** | 네트워크 차단, 5초 타임아웃, 5xx 에러 시 에러 노출 없이 정적 처방 출력 | ⬜ (Sprint 3) |
| **상태 초기화** | [다시하기] 클릭 시 슬라이더(50%) 및 원인 칩 초기화, 상단 스크롤 복귀 | ⬜ (Sprint 4) |
