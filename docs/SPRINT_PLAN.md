...
### 📝 작업 목록 (Task Breakdown)
1. **[Task 3-1]** 태그 미선택 시 API 차단 + Red 점멸 애니메이션 + 툴팁 노출
2. **[Task 3-2]** 중복 클릭 방지 (즉시 disabled + 스피너 전환 + Debounce)
3. **[Task 3-3]** 3초 응답 지연시 스켈레톤 유지 및 안내 문구 업데이트
4. **[Task 3-4]** 5초 타임아웃 / API 에러 / 5xx 장애 시 사용자 노출 없는 정적 JSON Fallback
5. **[Task 3-5]** 0% (긴급 응급 충전 배지 + 유령) 및 100% (유지 루틴 + Clamping) 극단 수치 처리
6. **[Task 3-6]** `navigator.onLine === false` 감지 시 오프라인 정적 처방 출력
7. **[Task 3-7]** 가로 800px 미만 시 `transform: scale()` 중앙 비율 정렬
...
### 📝 작업 목록 (Task Breakdown)
1. **[Task 4-1]** 힐링 루틴 체크박스 인터랙션 (체크표시 & 취소선 피드백)
2. **[Task 4-2]** `[👍 도움이 됐어요]` / `[👎 별로예요]` 피드백 버튼 및 2.2초 토스트 출력
3. **[Task 4-3]** `[🔄 다시하기]` 클릭 시 50% 슬라이더/태그 리셋 및 상단 스무스 스크롤 이동
4. **[Task 4-4]** DoD 체크리스트 7종 항목 종합 검증 및 QA 진행
